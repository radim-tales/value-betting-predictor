...
def test_loads_llm_section(tmp_path):
    p = tmp_path / "c.yaml"
    p.write_text(
        "league: E1\n"
        "seasons:\n  train: ['2122','2223']\n  validation: ['2324']\n  locked_test: ['2425']\n"
        ...
        "anchor: {type: elo_softmax_map, k: 20, home_adv: 70, start_rating: 1500}\n"
        "value: {min_edge: 0.03, odds_min: 1.6, odds_max: 4.5, skip_first_rounds: 4, stake: 1.0, max_bets_per_match: 1}\n"
        "llm: {correct_model: claude-haiku-4-5, reflect_model: claude-sonnet-5, temp_correct: 0.0, reflect_effort: medium}\n",
        encoding="utf-8",
    )
    cfg = load_config(p)
    assert cfg.llm.correct_model == "claude-haiku-4-5"
    assert cfg.llm.reflect_model == "claude-sonnet-5"
    assert cfg.llm.temp_correct == 0.0
    assert cfg.llm.reflect_effort == "medium"

def test_llm_section_optional_has_defaults(tmp_path):
    p = tmp_path / "c.yaml"
    p.write_text(
        "league: E1\n"
        "seasons:\n  train: ['2122','2223']\n  validation: ['2324']\n  locked_test: ['2425']\n"
